...
def InitializeModels(model_root,comm,models=[],verbose=False,filter=""):
...
def BuildLocalWorkList(M,C,comm):
...
def run_ilamb(model_root, config, models, confront, regions, clean, quiet, filter, build_dir, comm):
    ...
    T0 = time.time()
    M  = InitializeModels(model_root[0],comm,models,not quiet,filter=filter[0])
    if rank == 0 and not quiet: print "\nParsing config file %s...\n" % config[0]
    S = Scoreboard(config[0],
                   regions   = regions,
                   master    = rank==0,
                   verbose   = not quiet,
    ...
    f = FilterConfrontationList(C,confront)

    if rank == 0 and not quiet and len(Cf) != len(C):
    ...
    W = BuildLocalWorkList(M,C,comm)
    WorkConfront(W,not quiet,clean)
...
if __name__ == "__main__":
    ...
    run_ilamb(args.model_root, args.config, args.models, args.confront, args.regions, args.clean, args.quiet, args.filter, args.build_dir, comm)
